...
if fileitem != None:	
	with open(path + 'pic3.jpeg', 'wb+') as f:
		f.write(fileitem)
...
default_url = "localhost/cgi-bin/uploadfile.py"

comment = form.getvalue("comment", '')
def Comment_file():
    global comment 
    CommentsLog = open(path + "CommentsLogPic3.txt", 'a+')
    if comment == '':
        CommentsLog.close()
    else:
...
def read_from_file():
    ...
    line_counter = 0
    CommentsLog = open(path + "CommentsLogPic3.txt", 'r')
    for line in CommentsLog:
        ...
        if get_url() != default_url:
            ...
            if int(edit_line) == line_counter-1:
                ...
                print('<button type="submit" name="delete_comment" value="%d">X</button><button formmethod="get" formaction="/cgi-bin/uploadfile.py?edit_line=%d" type="submit" name="edit_line" value=%d>Edit</button><br></P>' % 
...
print('     <form action= "/cgi-bin/uploadfile.py" method= "post">')
print('         <P>Comments:</P>')
if os.environ['REQUEST_METHOD'] == 'POST':
    if comment == '':
